...
// #[query]
// fn get_profile_performance() -> Result<(), Error> {
//     // Retrieve the initial investment amount
//     let initial_investment = INITIAL_INVESTMENT.with(|investment_ref| *investment_ref.borrow().get());

//     // Get the current block index
//     let current_block = NEXT_BLOCK.with(|next_block_ref| *next_block_ref.borrow().get());

//     // Here you would typically query the ledger for the current balance
//     // For this example, we'll simulate a 5% increase
//     let current_value = (initial_investment as f64 * 1.05) as u64;

//     // Calculate ROI
//     let roi = (current_value as f64 - initial_investment as f64) / initial_investment as f64 * 100.0;
...
#[update]
async fn create_profile_investment(investment: Investment) -> Result<u64, Error> {
    // Convert amount to e8s (1 ICP = 100_000_000 e8s)
    let amount_e8s = (investment.amount * 100_000_000.0) as u64;

    // // Parse the destination address
    // let to_account = AccountIdentifier::from_hex(&investment.to)
    //     .map_err(|e| Error { message: format!("Invalid destination address: {}", e) })?;

    // // Prepare the transfer arguments
    // let transfer_args = TransferArgs {
    //     memo: Memo(investment.memo),
    //     amount: Tokens::from_e8s(amount_e8s),
    //     fee: Tokens::from_e8s(10_000), // Standard fee is 10_000 e8s
    //     from_subaccount: None, // Assuming transfer from the main account of the canister
    //     to: to_account,
    ...
    //     Ok(Ok(block_index)) => {
    //         // Update the NEXT_BLOCK
    //         NEXT_BLOCK.with(|next_block_ref| {
    //             let _ = next_block_ref.borrow_mut().set(block_index);
    //         });
    //         Ok(block_index)
    //     },
    //     Ok(Err(transfer_error)) => Err(Error { message: format!("Transfer error: {:?}", transfer_error) }),
    //     Err((code, msg)) => Err(Error { message: format!("Unexpected error: {:?}, {}", code, msg) }),
    // }
